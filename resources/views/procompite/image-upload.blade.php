...
<div id="container" class="container-fluid">
        ...
        <div class="col-md-12">
            <div class="jumbotron how-to-create" style="background-color: rgb(209,227,243)">
                ...
                <div class="row">
                    ...
                    <div class="col-md-1" style="display: none"><input type="radio" name="tipo" value="durante" checked> DURANTE<br></div>
                ...
                <form action="action('ProcompiteController@imgUpload')" class="dropzone" files="true" id ="dzone" >
    ...
    <!-- BOXES -->
    <h3>Galeria</h3><span id="foto_procompite"></span>

    <div id="procompite"></div>
    ...
    <script>
        ...
        $(function() {
            ...
            function btnState(){
                if($('#fecha').val() !== '' && actual + counter >= 1 && $('input[name=tipo]').is(':checked')===true ){
        ...
        cargarImg = function(){
            uid = document.getElementById('uid').value;
            $.get('procompite/server-images/' + uid.toString(), function(data) {
                //$('#antes').html('');
                $('#procompite').html('');
                //$('#despues').html('');

                console.log(data.procompite.length);
                if(data.procompite.length != 0){
                    $.each(data.procompite, function (key, value) {

                        url = value.url;
                        $('#procompite').append('<a style="margin-right:8px" class="fancybox" rel="group" href="'+url+'"><img width=150px height=150px src="'+url+'" alt="" /></a>');

                        $('#foto_procompite').text(' Fecha: ' + value.fecha);
                    });

                } else {
                    $('#procompite').append('<h4>No hay imagenes disponibles</h4>');
                }
