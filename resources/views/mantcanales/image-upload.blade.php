...
<div id="container" class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="jumbotron how-to-create" style="background-color: rgb(209,227,243)">
                ...
                <form action="action('MantCanalesController@imgUpload')" class="dropzone" files="true" id ="dzone" >
    ...
    <!-- BOXES -->
    <h3>Galeria</h3><span id="foto_canal"></span>
    ...
    <script>
        ...
        cargarImg = function(){
            uid = document.getElementById('uid').value;
            $.get('/mantcanales/server-images/' + uid.toString(), function(data) {
                //$('#antes').html('');
                $('#canal').html('');
                //$('#despues').html('');

                console.log(data.canal.length);
                if(data.canal.length != 0){
                    $.each(data.canal, function (key, value) {

                        url = value.url;
                        $('#canal').append('<a style="margin-right:8px" class="fancybox" rel="group" href="'+url+'"><img width=150px height=150px src="'+url+'" alt="" /></a>');

                        $('#foto_canal').text(' Fecha: ' + value.fecha);
                    });

                } else {
                    $('#canal').append('<h4>No hay imagenes disponibles</h4>');
                }
