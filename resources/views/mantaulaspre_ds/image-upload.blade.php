...
<div id="container" class="container-fluid">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="jumbotron how-to-create" style="background-color: rgb(209,227,243)">
                ...
                {!! Form::open(['action' => 'AulasPrefDSController@imgUpload', 'class' => 'dropzone', 'files'=>true, 'id'=>'dzone']) !!}
    ...
    <script>
        ...
        cargarImg = function(){
            uid = document.getElementById('uid').value;
            $.get('mantaulaspre/server-images/' + uid.toString(), function(data) {
                //$('#antes').html('');
                $('#aulaspre').html('');
                //$('#despues').html('');

                console.log(data.aulas_pre.length);
                if(data.aulas_pre.length != 0){
                    $.each(data.aulas_pre, function (key, value) {

                        url = value.url;
                        $('#aulaspre').append('<a style="margin-right:8px" class="fancybox" rel="group" href="'+url+'"><img width=150px height=150px src="'+url+'" alt="" /></a>');
                    });
