...
<div class="row" style="margin-bottom:15px;">
    <div class="col-sm-12">
        <h2 style="margin-top:0;">
            <span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span> Metas por Proyecto
        </h2>
    </div>
</div>
...
<div class="box box-info">
    ...
    <form method="POST" action="{{ URL::to('metas/proyecto/import') }}" enctype="multipart/form-data" id="form-import-proyecto">
        {{ csrf_field() }}
        <div class="box-body">
            <div class="row">
                <div class="col-md-8 form-group">
                    <label>Archivo</label>
                    <input type="file" name="file" id="file-proyecto" class="form-control" accept=".xlsx,.xls,.csv,.txt" required>
                </div>
                <div class="col-md-4 form-group">
                    <label>Formato esperado</label>
                    <p class="form-control-static">Columnas: anio, codigo_unico, m_enero ... m_diciembre, fecha_subida</p>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 form-group">
                    <label>Hoja a importar</label>
                    <select name="selected_sheet" id="selected-sheet-proyecto" class="form-control" required disabled>
        ...
        <div class="box-footer">
            <button type="submit" class="btn btn-info">Importar Meta Proyecto</button>
        </div>
    </form>
</div>
                ...
                @forelse($resumenes as $item)
                    <tr>
                        ...
                        <td style="white-space: nowrap;">
                            <form method="POST" action="{{ URL::to('metas/proyecto/delete-fecha') }}" style="display:inline;">
                                {{ csrf_field() }}
                                <input type="hidden" name="fecha_subida" value="{{ $item->fecha_subida }}">
                                <button type="submit" class="btn btn-xs btn-danger" onclick="return confirm('¿Seguro que desea eliminar este registro?');">
                                    Eliminar Fecha
                                </button>
                            </form>
                        </td>
...
<script>
$(function () {
    $('#file-proyecto').on('change', function () {
        var fileInput = this;
        var file = fileInput.files && fileInput.files[0] ? fileInput.files[0] : null;
        var $sheetSelect = $('#selected-sheet-proyecto');
        ...
        $.ajax({
            url: '{{ URL::to("metas/proyecto/sheets") }}',
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
