                        ...
                        <table id="proyectos_f12b_et" class="table table-striped table-bordered table-hover">
                            <thead style="background-color: #88b9d6">
                                <tr>
                                    ...
                                    <th style="vertical-align: middle;width:450px" class="text-center">NOMBRE PROYECTO</th>
                                    ...
                                    @endif
                                    <th style="vertical-align: middle;" class="text-center">DETALLE</th>
                            </thead>
                            <tbody>
                                @foreach($data as $row)
                                <tr>
                                    ...
                                    @endif
                                    <td style="vertical-align: middle;" class="text-center"><a style='cursor:pointer' onclick='loadModalDetalle("{{ $row->codigo_unico }}","{{ $filtro }}")'><i class="fa fa-tasks"></i></a></td>
                                </tr> 
                                @endforeach                               
                            </tbody>
                        </table>
...
<div class="pie">
    <script type="text/javascript">
        $(function(){
            ...
            loadModalDetalle = function(filtro,tipo) {
                    modaltype='full-width';
                    $modal = $('#' + modaltype);
                    $.ajax({
                        url: '{{ asset("/formato12b/showpryETDetalle") }}',
                        type: 'POST',
                        data:{codigo_unico:filtro,tipo:tipo},
                        beforeSend: function () {
                            $("#cargando").show();
                        },
