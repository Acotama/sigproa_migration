...
<html>
    ...
    <table>
        ...
        <tr id="header">
            ...
            <td class="header" width="110">DEVENGADO MES ACTUAL</td>
        ...
        @foreach($tipo as $item )
            @if(count($data->where($filtro,'=',$item)) > 0)
                @php
                    ...
                    $s_pia_dia = 0;
                    $s_pim_dia =0;
                    $s_certificacion_dia =0;
                    $s_comp_anual_dia =0;
                    $s_ate_comp_anual_dia =0;
                    $s_dev_dia =0;
                    $s_mes_dev =0;
                    $s_girado_dia =0;
                @endphp
                @foreach($data->where($filtro,'=',$item) as $key => $row )
                    @php
                        $s_cant_proyectos +=  1;
                        $s_m_pip += $row->m_pip;
                        $s_m_deveng_a += $row->m_deveng_a;
                        $s_pia_dia += $row->pia_dia;
                        $s_pim_dia += $row->pim_dia;
                        $s_certificacion_dia += $row->certificacion_dia;
                        $s_comp_anual_dia += $row->comp_anual_dia;
                        $s_ate_comp_anual_dia += $row->ate_comp_anual_dia;
                        $s_dev_dia += $row->dev_dia;
                        $s_mes_dev += $row->mes_dev;
                        $s_girado_dia += $row->girado_dia;
                    @endphp
                @endforeach
                <td class="total_num">{{ $s_cant_proyectos}}</td>
                <td class="total_text" colspan="2">{{ $item }}</td>
                <td class="total_num">{{ number_format($s_m_pip,2) }}</td>
                <td class="total_num">{{ number_format($s_m_deveng_a,2) }}</td>
                <td class="total_num">{{ number_format( ($s_m_deveng_a/$s_m_pip)*100,2) }}%</td>
                <td class="total_num">{{ number_format($s_pia_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_pim_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_certificacion_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_comp_anual_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_ate_comp_anual_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_dev_dia,2) }}</td>
                <td class="total_num">{{ number_format($s_mes_dev,2) }}</td>
                <td class="total_num">{{ number_format($s_girado_dia,2) }}</td>
                @if ($s_pim_dia == 0)
                    <td class="total_num">{{ number_format(0,2)}}%</td>
                @else
                    <td class="total_num">{{ number_format(($s_dev_dia/$s_pim_dia)* 100,2)}}%</td>
                @endif
