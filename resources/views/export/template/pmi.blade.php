        ...
        @foreach($tipo as $item )
            @if(count($data->where($filtro,'=',$item)) > 0)
                ...
                @foreach($data->where($filtro,'=',$item) as $key => $row )
                    @php
                        $s_cant_proyectos +=  1;
                        $s_m_pip += $row->costo_actualizado;
                        $s_m_deveng_a += $row->devengado_acumulado;
                        $s_pim_dia += $row->pim;
                        $s_pim_1 += $row->monto_1;
                        $s_pim_2 += $row->monto_2;
                        $s_pim_3 += $row->monto_3;
                ...
                <td class="total_text" colspan="10">{{ $item }}</td>
                ...
                <td class="total_num">{{ number_format($s_pim_1,2) }}</td>
                <td class="total_num">{{ number_format($s_pim_2,2) }}</td>
                <td class="total_num">{{ number_format($s_pim_3,2) }}</td>
                <td class="total_num">{{ number_format($s_pim_4,2) }}</td>
                @foreach($data->where($filtro,'=',$item) as $key => $row )
                    @php
                        $cant_proyectos +=  1;
