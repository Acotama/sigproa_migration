...
class ExportController extends Controller
{
        ...
        switch ($input['tipo']) {
            case 'piptotalpriori':
                ...
                foreach ($columns as $column) {
                    if ($colstr == "") {
                        $colstr .= 'grli_pip_total_priori.' . $column . ' as "' . utf8_encode($this->columns[$column]) . '"';
                    } else {
                        $colstr .= ",grli_pip_total_priori." . $column . ' as "' . utf8_encode($this->columns[$column]) . '"';
                    }
                }

                $pt = PipTotalPriori::select([DB::raw($colstr)])->where('id', '!=', '1000')->get()->toArray();

                return Excel::create('Sayhuite', function ($excel) use ($pt) {
                    $excel->sheet('Sayhuite', function ($sheet) use ($pt) {
                        $sheet->fromArray($pt);
    ...
    public function exportar_proyecto(Request $request)
    {
        ...
        $proyecto = DB::table("vw_data_clasificacion")->get();

        return response()
            ->view("export.template.proyecto", ['data' => $proyecto, 'filtro' => $request['filtro']], 200)
            ->header('Content-Description', 'File Transfer')
            ->header('Content-Type', 'text/html; charset=utf-8')
    ...
    public function reporte_diario_data_exportar()
    {
        ...
        //  Obtener los datos sin `CASE WHEN` en SQL
        $resumenPliego = DB::table("vw_grli_pip_seguimiento_ejecucion_financiera")
            ->select([
        ...
        //  Aplicar la clasificación en PHP
        $resumenPliego->transform(function ($item) use ($ordenGerencias) {
            $item->orden = isset($ordenGerencias[$item->ger_direc]) ? $ordenGerencias[$item->ger_direc] : 0;
            return $item;
        });

        //  Ordenar en PHP después de traer los datos
        $resumen_pliego = $resumenPliego->sortBy("orden")->values();

        //  Obtener los datos optimizados
        $resumenPliegoTotal = DB::table("vw_grli_pip_seguimiento_ejecucion_financiera")
            ->selectRaw("
            COUNT(*) as total_pry,
            SUM(pim_dia) as pim,
            SUM(dev_dia) as dev_dia
        ...
        //  Calcular el avance en PHP (más eficiente que `CASE WHEN` en SQL)
        $resumenPliegoTotal->avance = ($resumenPliegoTotal->pim > 0)
            ? ($resumenPliegoTotal->dev_dia * 100 / $resumenPliegoTotal->pim)
            : 0;

        $resumen_pliego_total = $resumenPliegoTotal;
    ...
    public function reporte_prueba(Request $request)
    {
        ...
        Excel::create('Reporte Diario', function ($excel) use ($data_reporte) {
            $excel->sheet('Reporte Diario', function ($sheet) use ($data_reporte) {
                ...
                // Informacion 
                    ...
                    $mapaMes = array(
                        ...
                        9 => ["m_setiembre", "cant_setiembre", "setiembre", "septiembre_certificado"],
                ...
                // Tamaño de las columnas de Filas
                    ...
                    $sheet->setHeight(2, 33);
                    $sheet->setHeight(3, 62);
                    $sheet->setHeight(4, 15);
                    $sheet->setHeight(5, 60);
                    $sheet->setHeight(6, 15);
                    $sheet->setHeight(7, 63);
                    $sheet->setHeight(8, 37);
                    $sheet->setHeight(9, 37);
                    $sheet->setHeight(10, 37);
                    $sheet->setHeight(11, 63);
                    $sheet->setHeight(12, 37);
                    $sheet->setHeight(13, 37);
                    $sheet->setHeight(14, 37);
                    $sheet->setHeight(15, 37);
                    $sheet->setHeight(16, 63);
                    $sheet->setHeight(17, 36);
                    $sheet->setHeight(18, 15);
                    $sheet->setHeight(19, 60);
                    $sheet->setHeight(20, 15);
                    $sheet->setHeight(21, 63);
                    $sheet->setHeight(22, 37);
                    $sheet->setHeight(23, 37);
                    $sheet->setHeight(24, 37);
                    $sheet->setHeight(25, 63);
                    $sheet->setHeight(26, 37);
                    $sheet->setHeight(27, 37);
                    $sheet->setHeight(28, 37);
                    $sheet->setHeight(29, 37);
                    $sheet->setHeight(30, 63);
                    $sheet->setHeight(31, 36);
                    $sheet->setHeight(32, 15);
                    $sheet->setHeight(33, 60);
                    $sheet->setHeight(34, 15);
                    $sheet->setHeight(35, 34);
                    $sheet->setHeight(36, 58);
                    $sheet->setHeight(37, 15);
                    $sheet->setHeight(38, 34);
                    $sheet->setHeight(39, 58);
                    $sheet->setHeight(40, 15);
                    $sheet->setHeight(41, 60);
                    $sheet->setHeight(42, 15);
                    $sheet->setHeight(43, 26);
                    $sheet->setHeight(44, 32);
                    $sheet->setHeight(45, 32);
                    $sheet->setHeight(46, 15);
                // Fin

                // Insertar imagen
                $drawing = new PHPExcel_Worksheet_Drawing();
                $drawing->setPath($logoPath);
                $drawing->setCoordinates('A2'); // Celda de inicio
                ...
                //  Ajustar tamaño exacto en píxeles
                ...
                $sheet->getStyle('C2:D2')->applyFromArray([
                    ...
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'A9D08E']],
                ...
                foreach ($resumen_pliego as $key => $row) {
                    ...
                    foreach ($columnas_porcentajes as $col) {
                        $sheet->getStyle("$col$fila")->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
                    }
                    ...
                    $styleC->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
                    $styleC->setWrapText(true);
                }
                // Total
                $sheet->setCellValue('B16', $resumen_pliego_total->total_pry);
                $sheet->mergeCells('C16:F16');
                $sheet->setCellValue('C16', 'TOTAL');
                $sheet->setCellValue('G16', $resumen_pliego_total->pim);
                $sheet->setCellValue('H16', $resumen_pliego_total->dev_dia);
                $sheet->setCellValue('I16', $resumen_pliego_total->pim - $resumen_pliego_total->dev_dia);
                $sheet->setCellValue('J16', $resumen_pliego_total->avance);
                $sheet->setCellValue('K16', $resumen_pliego_total->certificado);
                $sheet->setCellValue('L16', $resumen_pliego_total->avance_certificado);
                $sheet->getStyle('G16:I16')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('J16')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
                $sheet->getStyle('K16')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('L16')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
                $sheet->getStyle('B16:L16')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 26],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'A9D08E']],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ...
                foreach ($data_metas as $key => $row) {
                    ...
                    $meta_mes_2 = $row->{$meses[$mes]['m']};
                    $cantidad = isset($meses[$mes]['c']) ? $row->{$meses[$mes]['c']} : 0;
                    $dev_mes_2 = $row->{$meses[$mes]['d']};
                    $certificado_mes_2 = $row->{$meses[$mes]['cert']};
                    $pen_dev_acu_2 = array_reduce($meses[$mes]['pen'], function ($carry, $pen) use ($row) {
                        return $carry + $row->$pen;
                    }, 0);
                ...
                if ($t_1_meta_mes != 0) {
                    $sheet->setCellValue('J30', $t_1_devengado_mes * 100 / $t_1_meta_mes);
                } else {
                    $sheet->setCellValue('J30', 0);
                }
                $sheet->setCellValue('K30', $t_1_certificado_mes);
                $sheet->getStyle('G30:I30')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('J30')->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
                $sheet->getStyle('K30')->getNumberFormat()->setFormatCode('#,##0');
                ...
                // GRUPO 3
                $sheet->mergeCells('A32:N32');
                $sheet->mergeCells('A33:N33');
                ...
                $sheet->setCellValue('C35', "ENERO");
                $sheet->setCellValue('D35', "DEVENGADO");
                $sheet->setCellValue('D36', "PROGRAMACION");
                $sheet->setCellValue('E35', $historial_mensual->enero);
                $sheet->setCellValue('E36', $historial_mensual_meta->m_enero);
                $sheet->getStyle('C35:E36')->getAlignment()->setWrapText(true);
                $sheet->getStyle('C35:E36')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('E35:E36')->getFont()->setSize(24);
                $sheet->getStyle('C35:C36')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                // Febrero
                $sheet->mergeCells('G35:G36');
                $sheet->setCellValue('G35', "FEBRERO");
                $sheet->setCellValue('H35', "DEVENGADO");
                $sheet->setCellValue('H36', "PROGRAMACION");
                $sheet->setCellValue('I35', $historial_mensual->febrero);
                $sheet->setCellValue('I36', $historial_mensual_meta->m_febrero);
                $sheet->getStyle('G35:I36')->getAlignment()->setWrapText(true);
                $sheet->getStyle('G35:I36')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('I35:I36')->getFont()->setSize(24);
                $sheet->getStyle('G35:G36')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                // Marzo
                $sheet->mergeCells('K35:K36');
                $sheet->setCellValue('K35', "MARZO");
                $sheet->setCellValue('L35', "DEVENGADO");
                $sheet->setCellValue('L36', "PROGRAMACION");
                $sheet->setCellValue('M35', $historial_mensual->marzo);
                $sheet->setCellValue('M36', $historial_mensual_meta->m_marzo);
                $sheet->getStyle('K35:M36')->getAlignment()->setWrapText(true);
                $sheet->getStyle('K35:M36')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('M35:M36')->getFont()->setSize(24);
                $sheet->getStyle('K35:K36')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                // Abril
                $sheet->mergeCells('C38:C39');
                $sheet->setCellValue('C38', "ABRIL");
                $sheet->setCellValue('D38', "DEVENGADO");
                $sheet->setCellValue('D39', "PROGRAMACION");
                $sheet->setCellValue('E38', $historial_mensual->abril);
                $sheet->setCellValue('E39', $historial_mensual_meta->m_abril);
                $sheet->getStyle('C38:E39')->getAlignment()->setWrapText(true);
                $sheet->getStyle('C38:E39')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('E38:E39')->getFont()->setSize(24);
                $sheet->getStyle('C38:C39')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                // Mayo
                $sheet->mergeCells('G38:G39');
                $sheet->setCellValue('G38', "MAYO");
                $sheet->setCellValue('H38', "DEVENGADO");
                $sheet->setCellValue('H39', "PROGRAMACION");
                $sheet->setCellValue('I38', $historial_mensual->mayo);
                $sheet->setCellValue('I39', $historial_mensual_meta->m_mayo);
                $sheet->getStyle('G38:I39')->getAlignment()->setWrapText(true);
                $sheet->getStyle('G38:I39')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('I38:I39')->getFont()->setSize(24);
                $sheet->getStyle('G38:G39')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                // Junio
                $sheet->mergeCells('K38:K39');
                $sheet->setCellValue('K38', "JUNIO");
                $sheet->setCellValue('L38', "DEVENGADO");
                $sheet->setCellValue('L39', "PROGRAMACION");
                $sheet->setCellValue('M38', $historial_mensual->junio);
                $sheet->setCellValue('M39', $historial_mensual_meta->m_junio);
                $sheet->getStyle('K38:M39')->getAlignment()->setWrapText(true);
                $sheet->getStyle('K38:M39')->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getStyle('M38:M39')->getFont()->setSize(24);
                $sheet->getStyle('K38:K39')->applyFromArray([
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                ]);
                $fila_inicio = 40;
                $sheet->getStyle('E35:E36')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('I35:I36')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('M35:M36')->getNumberFormat()->setFormatCode('#,##0');

                $sheet->getStyle('E38:E39')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('I38:I39')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->getStyle('M38:M39')->getNumberFormat()->setFormatCode('#,##0');
                $sheet->setHeight(47, 60);
                $sheet->setHeight(48, 15);
                $sheet->setHeight(49, 27);
                $sheet->setHeight(50, 54);
                if ($mes > 6) {
                    $fila_inicio = 46;
                    $sheet->mergeCells('A40:N40');
                    // Julio
                    $sheet->mergeCells('C41:C42');
                    $sheet->setCellValue('C41', "JULIO");
                    $sheet->setCellValue('D41', "DEVENGADO");
                    $sheet->setCellValue('D42', "PROGRAMACION");
                    $sheet->setCellValue('E41', $historial_mensual->julio);
                    $sheet->setCellValue('E42', $historial_mensual_meta->m_julio);
                    $sheet->getStyle('C41:E42')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('C41:E42')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('E41:E42')->getFont()->setSize(24);
                    $sheet->getStyle('C41:C42')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    // Agosto
                    $sheet->mergeCells('G41:G42');
                    $sheet->setCellValue('G41', "AGOSTO");
                    $sheet->setCellValue('H41', "DEVENGADO");
                    $sheet->setCellValue('H42', "PROGRAMACION");
                    $sheet->setCellValue('I41', $historial_mensual->agosto);
                    $sheet->setCellValue('I42', $historial_mensual_meta->m_agosto);
                    $sheet->getStyle('G41:I42')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('G41:I42')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('I41:I42')->getFont()->setSize(24);
                    $sheet->getStyle('G41:G42')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    // Septiembre
                    $sheet->mergeCells('K41:K42');
                    $sheet->setCellValue('K41', "SEPTIEMBRE");
                    $sheet->setCellValue('L41', "DEVENGADO");
                    $sheet->setCellValue('L42', "PROGRAMACION");
                    $sheet->setCellValue('M41', $historial_mensual->septiembre);
                    $sheet->setCellValue('M42', $historial_mensual_meta->m_setiembre);
                    $sheet->getStyle('K41:M42')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('K41:M42')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('M41:M42')->getFont()->setSize(24);
                    $sheet->getStyle('K41:K42')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    // Octubre
                    $sheet->mergeCells('C44:C45');
                    $sheet->setCellValue('C44', "OCTUBRE");
                    $sheet->setCellValue('D44', "DEVENGADO");
                    $sheet->setCellValue('D45', "PROGRAMACION");
                    $sheet->setCellValue('E44', $historial_mensual->octubre);
                    $sheet->setCellValue('E45', $historial_mensual_meta->m_octubre);
                    $sheet->getStyle('C44:E45')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('C44:E45')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('E44:E45')->getFont()->setSize(24);
                    $sheet->getStyle('C44:C45')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    // Noviembre
                    $sheet->mergeCells('G44:G45');
                    $sheet->setCellValue('G44', "NOVIEMBRE");
                    $sheet->setCellValue('H44', "DEVENGADO");
                    $sheet->setCellValue('H45', "PROGRAMACION");
                    $sheet->setCellValue('I44', $historial_mensual->noviembre);
                    $sheet->setCellValue('I45', $historial_mensual_meta->m_noviembre);
                    $sheet->getStyle('G44:I45')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('G44:I45')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('I44:I45')->getFont()->setSize(24);
                    $sheet->getStyle('G44:G45')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    // Diciembre
                    $sheet->mergeCells('K44:K45');
                    $sheet->setCellValue('K44', "DICIEMBRE");
                    $sheet->setCellValue('L44', "DEVENGADO");
                    $sheet->setCellValue('L45', "PROGRAMACION");
                    $sheet->setCellValue('M44', $historial_mensual->diciembre);
                    $sheet->setCellValue('M45', $historial_mensual_meta->m_diciembre);
                    $sheet->getStyle('K44:M45')->getAlignment()->setWrapText(true);
                    $sheet->getStyle('K44:M45')->applyFromArray([
                        'font' => ['bold' => true, 'size' => 22],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('M44:M45')->getFont()->setSize(24);
                    $sheet->getStyle('K44:K45')->applyFromArray([
                        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D9D9D9']],
                    ]);
                    $sheet->setHeight(41, 34);
                    $sheet->setHeight(42, 58);
                    $sheet->setHeight(43, 15);
                    $sheet->setHeight(44, 34);
                    $sheet->setHeight(45, 58);
                    $sheet->setHeight(46, 15);
                    $sheet->getStyle('E41:E42')->getNumberFormat()->setFormatCode('#,##0');
                    $sheet->getStyle('I41:I42')->getNumberFormat()->setFormatCode('#,##0');
                    $sheet->getStyle('M41:M42')->getNumberFormat()->setFormatCode('#,##0');

                    $sheet->getStyle('E44:E45')->getNumberFormat()->setFormatCode('#,##0');
                    $sheet->getStyle('I44:I45')->getNumberFormat()->setFormatCode('#,##0');
                    $sheet->getStyle('M44:M45')->getNumberFormat()->setFormatCode('#,##0');
                }
                // FIN GRUPO 3

                // GRUPO 4
                $sheet->mergeCells('A' . $fila_inicio . ':N' . $fila_inicio);
                $sheet->mergeCells('A' . ($fila_inicio + 1) . ':N' . ($fila_inicio + 1));
                $sheet->setCellValue('A' . ($fila_inicio + 1), "PROGRAMACION MEF vs FORMATO 12-B");
                $sheet->getStyle('A' . ($fila_inicio + 1))->applyFromArray([
                    'font' => ['bold' => true, 'size' => 48],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'A9D08E']],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ...
                $sheet->setCellValue('B' . ($fila_inicio + 3), "ENERO");
                $sheet->setCellValue('C' . ($fila_inicio + 3), "FEBRERO");
                $sheet->setCellValue('D' . ($fila_inicio + 3), "MARZO");
                $sheet->setCellValue('E' . ($fila_inicio + 3), "ABRIL");
                $sheet->setCellValue('F' . ($fila_inicio + 3), "MAYO");
                $sheet->setCellValue('G' . ($fila_inicio + 3), "JUNIO");
                $sheet->setCellValue('H' . ($fila_inicio + 3), "JULIO");
                $sheet->setCellValue('I' . ($fila_inicio + 3), "AGOSTO");
                $sheet->setCellValue('J' . ($fila_inicio + 3), "SEPTIEMBRE");
                $sheet->setCellValue('K' . ($fila_inicio + 3), "OCTUBRE");
                $sheet->setCellValue('L' . ($fila_inicio + 3), "NOVIEMBRE");
                $sheet->setCellValue('M' . ($fila_inicio + 3), "DICIEMBRE");
                $sheet->setCellValue('N' . ($fila_inicio + 3), "TOTAL");
                $sheet->getStyle('B' . ($fila_inicio + 3) . ':N' . ($fila_inicio + 3))->applyFromArray([
                ...
                foreach ($mefvs_formato12b as $key => $row) {
                    ...
                    $sheet->setCellValue('A' . ($fila_inicio + $key + 4), $row->info);
                    $sheet->setCellValue('B' . ($fila_inicio + $key + 4), $row->a_enero);
                    $sheet->setCellValue('C' . ($fila_inicio + $key + 4), $row->a_febrero);
                    $sheet->setCellValue('D' . ($fila_inicio + $key + 4), $row->a_marzo);
                    $sheet->setCellValue('E' . ($fila_inicio + $key + 4), $row->a_abril);
                    $sheet->setCellValue('F' . ($fila_inicio + $key + 4), $row->a_mayo);
                    $sheet->setCellValue('G' . ($fila_inicio + $key + 4), $row->a_junio);
                    $sheet->setCellValue('H' . ($fila_inicio + $key + 4), $row->a_julio);
                    $sheet->setCellValue('I' . ($fila_inicio + $key + 4), $row->a_agosto);
                    $sheet->setCellValue('J' . ($fila_inicio + $key + 4), $row->a_setiembre);
                    $sheet->setCellValue('K' . ($fila_inicio + $key + 4), $row->a_octubre);
                    $sheet->setCellValue('L' . ($fila_inicio + $key + 4), $row->a_noviembre);
                    $sheet->setCellValue('M' . ($fila_inicio + $key + 4), $row->a_diciembre);
                    $sheet->setCellValue('N' . ($fila_inicio + $key + 4), $total_meta);
                    $sheet->getStyle('A' . ($fila_inicio + $key + 4) . ':N' . ($fila_inicio + $key + 4))->applyFromArray([
                        'font' => ['bold' => true, 'size' => 24],
                        'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                        'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                    ]);
                    $sheet->getStyle('A' . ($fila_inicio + $key + 4))->getFont()->setSize(20);
                    $sheet->getStyle('B' . ($fila_inicio + $key + 4) . ':N' . ($fila_inicio + $key + 5))->getNumberFormat()->setFormatCode('#,##0');
                }
                // FIN GRUPO 4

                // GRUPO 5
                $fila_inicio = $fila_inicio + 6;
                $sheet->mergeCells('A' . $fila_inicio . ':N' . $fila_inicio);
                $sheet->mergeCells('A' . ($fila_inicio + 1) . ':N' . ($fila_inicio + 1));
                $sheet->setCellValue('A' . ($fila_inicio + 1), "RESUMEN DE EJECUCION POR PROYECTO AL MES DE " . $mes_nombre);
                $sheet->getStyle('A' . ($fila_inicio + 1))->applyFromArray([
                    'font' => ['bold' => true, 'size' => 48],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'A9D08E']],
                ...
                $sheet->getStyle('A' . ($fila_inicio + 3) . ':N' . ($fila_inicio + 4))->getAlignment()->setWrapText(true);
                ...
                $sheet->getStyle('A' . ($fila_inicio + 3) . ':N' . ($fila_inicio + 4))->applyFromArray([
                    'font' => ['bold' => true, 'size' => 20],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'D3EBF7']],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ]);
                $sheet->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd($fila_inicio + 3, $fila_inicio + 4);
                $fila_inicio = $fila_inicio + 5;
                foreach ($uei as $item) {
                    ...
                    if (count($data_hoy_a) > 0) {
                        ...
                        if ($programacion_t > 0) {
                            $sheet->setCellValue('H' . ($fila_inicio), ($ejecucion_t * 100 / $programacion_t));
                        } else {
                            $sheet->setCellValue('H' . ($fila_inicio), 0);
                        }
                        $sheet->setCellValue('I' . ($lunes_t), 0);
                        $sheet->setCellValue('J' . ($martes_t), 0);
                        $sheet->setCellValue('K' . ($miercoles_t), 0);
                        $sheet->setCellValue('L' . ($jueves_t), 0);
                        $sheet->setCellValue('M' . ($viernes_t), 0);
                        $sheet->setCellValue('N' . ($total_semana_t), 0);
                        $sheet->getStyle('A' . ($fila_inicio) . ':N' . ($fila_inicio))->applyFromArray([
                            'font' => ['bold' => true, 'size' => 20],
                            'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                            'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => '9BC2E6']],
                            'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                        ]);
                        ...
                        $sheet->setHeight($fila_inicio, 35);
                        foreach ($data_hoy_meta as $key => $row) {
                            ...
                            if ($meta_mes > 0) {
                                $sheet->setCellValue('H' . ($fila_inicio), $row->dev_mes * 100 / $meta_mes);
                            } else {
                                $sheet->setCellValue('H' . ($fila_inicio), 0);
                            }
                            if (count($lunes) > 0) {
                                $sheet->setCellValue('I' . ($fila_inicio), $lunes->dif_dev_dia);
                                $total_semana += $lunes->dif_dev_dia;
                            } else {
                                $sheet->setCellValue('I' . ($fila_inicio), 0);
                            }
                            if (count($martes) > 0) {
                                $sheet->setCellValue('J' . ($fila_inicio), $martes->dif_dev_dia);
                                $total_semana += $martes->dif_dev_dia;
                            } else {
                                $sheet->setCellValue('J' . ($fila_inicio), 0);
                            }
                            if (count($miercoles) > 0) {
                                $sheet->setCellValue('K' . ($fila_inicio), $miercoles->dif_dev_dia);
                                $total_semana += $miercoles->dif_dev_dia;
                            } else {
                                $sheet->setCellValue('K' . ($fila_inicio), 0);
                            }
                            if (count($jueves) > 0) {
                                $sheet->setCellValue('L' . ($fila_inicio), $jueves->dif_dev_dia);
                                $total_semana += $jueves->dif_dev_dia;
                            } else {
                                $sheet->setCellValue('L' . ($fila_inicio), 0);
                            }
                            if (count($viernes) > 0) {
                                $sheet->setCellValue('M' . ($fila_inicio), $viernes->dif_dev_dia);
                                $total_semana += $viernes->dif_dev_dia;
                            } else {
                                $sheet->setCellValue('M' . ($fila_inicio), 0);
                            }
                            $sheet->setCellValue('N' . ($fila_inicio), $total_semana);
                            $sheet->getStyle('F' . ($fila_inicio) . ':G' . ($fila_inicio))->getNumberFormat()->setFormatCode('#,##0');
                            $sheet->getStyle('H' . ($fila_inicio))->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
                            $sheet->getStyle('I' . ($fila_inicio) . ':N' . ($fila_inicio))->getNumberFormat()->setFormatCode('#,##0');
                            $sheet->setHeight($fila_inicio, 80);
                            $sheet->getStyle('A' . ($fila_inicio) . ':N' . ($fila_inicio))->applyFromArray([
                    ...
                    if (count($data_hoy_adicional) > 0) {
                        ...
                        $sheet->getStyle('A' . ($fila_inicio) . ':N' . ($fila_inicio))->applyFromArray([
                            ...
                            'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                            'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'C4D79B']],
                            'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                        ]);
                        $sheet->setHeight($fila_inicio, 30);
                        $fila_inicio = $fila_inicio + 1;
                ...
                if ($t_programacion > 0) {
                    $sheet->setCellValue('H' . ($fila_inicio), ($t_ejecucion * 100 / $t_programacion));
                } else {
                    $sheet->setCellValue('H' . ($fila_inicio), 0);
                }
                $sheet->setCellValue('I' . ($t_lunes), 0);
                $sheet->setCellValue('J' . ($t_martes), 0);
                $sheet->setCellValue('K' . ($t_miercoles), 0);
                $sheet->setCellValue('L' . ($t_jueves), 0);
                $sheet->setCellValue('M' . ($t_viernes), 0);
                $sheet->setCellValue('N' . ($t_semana_total), 0);
                $sheet->getStyle('A' . ($fila_inicio) . ':N' . ($fila_inicio))->applyFromArray([
                    'font' => ['bold' => true, 'size' => 20],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'startcolor' => ['rgb' => 'A9D08E']],
                ...
                $sheet->getStyle('H' . ($fila_inicio))->getNumberFormat()->setFormatCode(NumberFormat::FORMAT_NUMBER_00);
                $sheet->getStyle('I' . ($fila_inicio) . ':N' . ($fila_inicio))->getNumberFormat()->setFormatCode('#,##0');
                $sheet->setHeight($fila_inicio, 40);
                $sheet->getStyle('A' . ($fila_inicio) . ':N' . ($fila_inicio))->applyFromArray([
                    'font' => ['bold' => true, 'size' => 22],
                    'alignment' => ['vertical' => 'center', 'horizontal' => 'center'],
                    'borders' => ['allborders' => ['style' => Border::BORDER_THIN, 'color' => ['rgb' => '000000']]],
                ...
                $sheet->setPrintArea('A1:N' . $fila_inicio);
    ...
    public function reporte_diario_fin_mes2(Request $request)
    {
        $data = DB::table("vw_pry_dev_ejecutoras_total")->select(DB::RAW("sum(cantida) as cantida,
            sum(pim_dia) as pim_dia,
            sum(certificacion_dia) as certificacion_dia,
